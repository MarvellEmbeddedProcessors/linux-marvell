...
#define DRV_NAME	"rvu-nic-bphypf"
...
static const struct otx2_bphypf_stat otx2_dev_stats[] = {
...
static const struct otx2_bphypf_stat otx2_drv_stats[] = {
...
static const struct otx2_bphypf_stat otx2_queue_stats[] = {
	{ "bytes", 0 },
	{ "frames", 1 },
};
...
static struct cgx_fw_data *otx2_bphypf_get_fwdata(struct otx2_nic *pfvf);

static void otx2_bphypf_get_drvinfo(struct net_device *netdev,
...
static void otx2_bphypf_get_qset_strings(struct otx2_nic *pfvf, u8 **data,
					 int qset)
{
...
static void otx2_bphypf_get_strings(struct net_device *netdev, u32 sset,
				    u8 *data)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
	int stats;
	...
	otx2_bphypf_get_qset_strings(pfvf, &data, 0);
...
static void otx2_bphypf_get_qset_stats(struct otx2_nic *pfvf,
...
static int otx2_bphypf_get_phy_fec_stats(struct otx2_nic *pfvf)
{
	struct msg_req *req;
	int rc = -ENOMEM;
...
static void otx2_bphypf_get_ethtool_stats(struct net_device *netdev,
					  struct ethtool_stats *stats, u64 *data)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
	...
	otx2_bphypf_get_qset_stats(pfvf, stats, &data);
	...
	*(data++) = pfvf->reset_count;
	...
	if (!IS_ERR(rsp) && rsp->fwdata.phy.misc.has_fec_stats &&
	    !otx2_bphypf_get_phy_fec_stats(pfvf)) {
...
static int otx2_bphypf_get_sset_count(struct net_device *netdev, int sset)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static void otx2_bphypf_get_channels(struct net_device *dev,
...
static int otx2_bphypf_set_channels(struct net_device *dev,
				    struct ethtool_channels *channel)
{
	struct otx2_nic *pfvf = netdev_priv(dev);
...
static void otx2_bphypf_get_pauseparam(struct net_device *netdev,
...
static int otx2_bphypf_set_pauseparam(struct net_device *netdev,
				      struct ethtool_pauseparam *pause)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static void otx2_bphypf_get_ringparam(struct net_device *netdev,
...
static int otx2_bphypf_set_ringparam(struct net_device *netdev,
				     struct ethtool_ringparam *ring,
				     struct kernel_ethtool_ringparam *kernel_ring,
				     struct netlink_ext_ack *extack)
{
...
static int otx2_bphypf_get_coalesce(struct net_device *netdev,
...
static int otx2_bphypf_set_coalesce(struct net_device *netdev,
...
static int otx2_bphypf_get_rss_hash_opts(struct otx2_nic *pfvf,
					 struct ethtool_rxnfc *nfc)
{
	...
	/* Minimum is IPv4 and IPv6, SIP/DIP */
...
static int otx2_bphypf_set_rss_hash_opts(struct otx2_nic *pfvf,
					 struct ethtool_rxnfc *nfc)
{
	struct otx2_rss_info *rss = &pfvf->hw.rss_info;
...
static int otx2_bphypf_get_rxnfc(struct net_device *dev,
				 struct ethtool_rxnfc *nfc, u32 *rules)
{
	...
	case ETHTOOL_GRXCLSRLALL:
		...
		break;
	case ETHTOOL_GRXFH:
		return otx2_bphypf_get_rss_hash_opts(pfvf, nfc);
	default:
		break;
	}
	return ret;
...
static int otx2_bphypf_set_rxnfc(struct net_device *dev, struct ethtool_rxnfc *nfc)
{
	bool ntuple = !!(dev->features & NETIF_F_NTUPLE);
	struct otx2_nic *pfvf = netdev_priv(dev);
	int ret = -EOPNOTSUPP;
	...
	switch (nfc->cmd) {
	case ETHTOOL_SRXFH:
		ret = otx2_bphypf_set_rss_hash_opts(pfvf, nfc);
		break;
...
static u32 otx2_bphypf_get_rxfh_key_size(struct net_device *netdev)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static u32 otx2_bphypf_get_rxfh_indir_size(struct net_device *dev)
{
...
static int otx2_bphypf_set_rxfh_context(struct net_device *dev,
					const u32 *indir,
...
static int otx2_bphypf_get_rxfh_context(struct net_device *dev, u32 *indir,
...
static int otx2_bphypf_get_rxfh(struct net_device *dev, u32 *indir,
				u8 *hkey, u8 *hfunc)
{
	return otx2_bphypf_get_rxfh_context(dev, indir, hkey, hfunc,
				     DEFAULT_RSS_CONTEXT_GROUP);
}

/* Configure RSS table and hash key */
static int otx2_bphypf_set_rxfh(struct net_device *dev, const u32 *indir,
				const u8 *hkey, const u8 hfunc)
{
	u32 rss_context = DEFAULT_RSS_CONTEXT_GROUP;

	return otx2_bphypf_set_rxfh_context(dev, indir, hkey, hfunc,
					    &rss_context, 0);
}

static u32 otx2_bphypf_get_msglevel(struct net_device *netdev)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static void otx2_bphypf_set_msglevel(struct net_device *netdev, u32 val)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static u32 otx2_bphypf_get_link(struct net_device *netdev)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static int otx2_bphypf_get_ts_info(struct net_device *netdev,
...
static int otx2_bphypf_get_fecparam(struct net_device *netdev,
...
static int otx2_bphypf_set_fecparam(struct net_device *netdev,
				    struct ethtool_fecparam *fecparam)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
...
static void otx2_bphypf_get_fec_info(u64 index, int req_mode,
...
static void otx2_bphypf_get_link_mode_info(u64 link_mode_bmap,
					   bool req_mode,
...
static int otx2_bphypf_get_module_info(struct net_device *netdev,
...
static int otx2_bphypf_get_module_eeprom(struct net_device *netdev,
...
static int otx2_bphypf_get_link_ksettings(struct net_device *netdev,
					  struct ethtool_link_ksettings *cmd)
{
	...
	otx2_bphypf_get_link_mode_info(rsp->fwdata.advertised_link_modes,
				       OTX2_MODE_ADVERTISED, cmd);
	otx2_bphypf_get_fec_info(rsp->fwdata.advertised_fec,
				 OTX2_MODE_ADVERTISED, cmd);
	otx2_bphypf_get_link_mode_info(rsp->fwdata.supported_link_modes,
				       OTX2_MODE_SUPPORTED, cmd);
	otx2_bphypf_get_fec_info(rsp->fwdata.supported_fec,
				 OTX2_MODE_SUPPORTED, cmd);
	return 0;
}

static int otx2_bphypf_set_link_ksettings(struct net_device *netdev,
					  const struct ethtool_link_ksettings *cmd)
{
	...
	otx2_bphypf_get_link_ksettings(netdev, &cur_ks);
...
static u32 otx2_bphypf_get_priv_flags(struct net_device *netdev)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
	struct cgx_fw_data *rsp;
...
static int otx2_bphypf_set_phy_mod_type(struct net_device *netdev, bool enable)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
	...
	fwd = otx2_bphypf_get_fwdata(pfvf);
	if (IS_ERR(fwd))
		return -EAGAIN;
				   ...
				   u32 len, bool enable)
{
	...
	/* In case HIGIG2 mode is set packet will have 16 bytes of
...
static int otx2_bphypf_set_priv_flags(struct net_device *netdev, u32 new_flags)
{
	struct otx2_nic *pfvf = netdev_priv(netdev);
	bool enable = false;
	int bitnr, rc = 0;
	...
	otx2_bphypf_get_priv_flags(netdev);
	...
	if ((BIT(bitnr) != OTX2_PRIV_FLAG_PAM4) &&
	...
	case OTX2_PRIV_FLAG_PAM4:
		rc = otx2_bphypf_set_phy_mod_type(netdev, enable);
		break;
...
static const struct ethtool_ops otx2_bphypf_ethtool_ops = {
	.supported_coalesce_params = ETHTOOL_COALESCE_USECS |
				     ETHTOOL_COALESCE_MAX_FRAMES |
				     ETHTOOL_COALESCE_USE_ADAPTIVE,
	.supported_ring_params  = ETHTOOL_RING_USE_RX_BUF_LEN |
				  ETHTOOL_RING_USE_CQE_SIZE,
	.get_link		= otx2_bphypf_get_link,
	.get_drvinfo		= otx2_bphypf_get_drvinfo,
	.get_strings		= otx2_bphypf_get_strings,
	.get_ethtool_stats	= otx2_bphypf_get_ethtool_stats,
	.get_sset_count		= otx2_bphypf_get_sset_count,
	.set_channels		= otx2_bphypf_set_channels,
	.get_channels		= otx2_bphypf_get_channels,
	.get_ringparam		= otx2_bphypf_get_ringparam,
	.set_ringparam		= otx2_bphypf_set_ringparam,
	.get_coalesce		= otx2_bphypf_get_coalesce,
	.set_coalesce		= otx2_bphypf_set_coalesce,
	.get_rxnfc		= otx2_bphypf_get_rxnfc,
	.set_rxnfc              = otx2_bphypf_set_rxnfc,
	.get_rxfh_key_size	= otx2_bphypf_get_rxfh_key_size,
	.get_rxfh_indir_size	= otx2_bphypf_get_rxfh_indir_size,
	.get_rxfh		= otx2_bphypf_get_rxfh,
	.set_rxfh		= otx2_bphypf_set_rxfh,
	.get_rxfh_context	= otx2_bphypf_get_rxfh_context,
	.set_rxfh_context	= otx2_bphypf_set_rxfh_context,
	.get_msglevel		= otx2_bphypf_get_msglevel,
	.set_msglevel		= otx2_bphypf_set_msglevel,
	.get_pauseparam		= otx2_bphypf_get_pauseparam,
	.set_pauseparam		= otx2_bphypf_set_pauseparam,
	.get_ts_info		= otx2_bphypf_get_ts_info,
	.get_fecparam		= otx2_bphypf_get_fecparam,
	.set_fecparam		= otx2_bphypf_set_fecparam,
	.get_link_ksettings     = otx2_bphypf_get_link_ksettings,
	.set_link_ksettings     = otx2_bphypf_set_link_ksettings,
	.get_priv_flags		= otx2_bphypf_get_priv_flags,
	.set_priv_flags		= otx2_bphypf_set_priv_flags,
	.get_module_info	= otx2_bphypf_get_module_info,
	.get_module_eeprom	= otx2_bphypf_get_module_eeprom,
};

void otx2_bphypf_set_ethtool_ops(struct net_device *netdev)
{
	netdev->ethtool_ops = &otx2_bphypf_ethtool_ops;
}

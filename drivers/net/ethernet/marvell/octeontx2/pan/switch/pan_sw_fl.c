...
static int
pan_sw_fl_stats_add_node(unsigned long cookie, u16 mcam_idx[2])
{
...
static int pan_sw_fl_hw_del_n_free(u16 *mcam_idx)
{
	...
	req->cnt = 2;
	req->entry[0] = mcam_idx[0];
	req->entry[1] = mcam_idx[1];
...
static int pan_sw_fl_del(unsigned long cookie)
{
	...
	pan_sw_fl_hw_del_n_free(fl->mcam_idx);
...
static int pan_sw_fl_hw_alloc_mcam_entry(u16 *mcam_idx)
{
	...
	mcam_idx[0] = rsp->entry_list[0];
	mcam_idx[1] = rsp->entry_list[1];
...
static int pan_sw_fl_hw_flow_install(u16 mcam_idx, struct fl_tuple *tuple, u16 match_id)
{
...
static void
pan_sw_fl_get_reply_tuple(struct fl_tuple *tuple)
{
	u8 eth_addr[16];
	u32 ip_addr;
	u16 port;
...
static int pan_sw_fl_tbl_entry_add(struct fl_tuple *ftuple, u16 match_id, u8 dir)
{
	...
	/* We expect npc rules NOT mask based */
	opq.eg_dmac_can_set = 1;

	if (ftuple->mangle_valid & BIT(FLOW_ACT_MANGLE_HDR_TYPE_ETH)) {
		if (ftuple->mangle[FLOW_ACT_MANGLE_HDR_TYPE_ETH].val) {
			res.act = PAN_FL_TBL_ACT_L3_FWD;

			if (ftuple->is_xdev_br)
				res.act = PAN_FL_TBL_ACT_L3_BR_FWD;
		}
	}

	if (ftuple->mangle_valid & BIT(FLOW_ACT_MANGLE_HDR_TYPE_IP4)) {
		if (ftuple->mangle[FLOW_ACT_MANGLE_HDR_TYPE_IP4].val) {
			opq.eg_sip = ftuple->mangle[FLOW_ACT_MANGLE_HDR_TYPE_IP4].val;
			offset = ftuple->mangle[FLOW_ACT_MANGLE_HDR_TYPE_IP4].offset;
			if (offset == 12) {
				res.act = PAN_FL_TBL_ACT_L3_SNAT;

				if (ftuple->is_xdev_br)
					res.act = PAN_FL_TBL_ACT_L3_BR_SNAT;
			} else if (offset == 16) {
				res.act = PAN_FL_TBL_ACT_L3_DNAT;

				if (ftuple->is_xdev_br)
					res.act = PAN_FL_TBL_ACT_L3_BR_DNAT;
			}
		}
	}
...
static int pan_sw_fl_add(unsigned long cookie, struct fl_tuple *ftuple)
{
	...
	rc = pan_sw_fl_tbl_entry_add(ftuple, match_id[0], IP_CT_DIR_ORIGINAL);
	if (rc) {
	...
	reply_tuple = *ftuple;
	pan_sw_fl_get_reply_tuple(&reply_tuple);

	rc = pan_sw_fl_tbl_entry_add(&reply_tuple, match_id[1], IP_CT_DIR_REPLY);
	if (rc) {
		pr_err("%s:%d Error install flow tuple\n", __func__, __LINE__);
		goto free;
	}

	rc = pan_sw_fl_hw_alloc_mcam_entry(mcam_idx);
	if (rc) {
		pr_err("%s:%d Error to alloc mcam idxs\n", __func__, __LINE__);
		goto free;
	}

	rc = pan_sw_fl_hw_flow_install(mcam_idx[0], ftuple, match_id[0]);
	if (rc) {
		pr_err("%s:%d Error to install original dir flow", __func__, __LINE__);
		goto free;
	}

	rc = pan_sw_fl_hw_flow_install(mcam_idx[1], &reply_tuple, match_id[1]);
	if (rc) {
		pr_err("%s:%d Error to install reply dir flow", __func__, __LINE__);
		goto free;
	}

	rc = pan_sw_fl_stats_add_node(cookie, mcam_idx);
	if (rc) {
...
static void pan_sw_fl_dwork(struct work_struct *dwork)
{
	...
	for (int i = 0; i < cnt; i++) {
		req->cookie[i] = cookie[i];
		req->mcam_idx[i][0] = mcam_idx[i][0];
		req->mcam_idx[i][1] = mcam_idx[i][1];
		req->dis[i] = disabled[i];
	}

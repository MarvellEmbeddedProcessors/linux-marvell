...
static irqreturn_t psw_evf_flr_done_intr_handler(int irq, void *ptr)
{
	...
	reg = rvu_read64(rvu, blkaddr, PSW_AF_EVFX_FLR_DONE_INT(evf_id));

	rvu_write64(rvu, blkaddr, PSW_AF_EVFX_FLR_DONE_INT(evf_id), reg);

	return IRQ_HANDLED;
}

static irqreturn_t psw_epf_flr_done_intr_handler(int irq, void *ptr)
{
	struct rvu_block *block = ptr;
	struct rvu *rvu = block->rvu;
	int blkaddr = block->addr;
	...
	reg = rvu_read64(rvu, blkaddr, PSW_AF_EPF_FLR_DONE_INT);

	rvu_write64(rvu, blkaddr, PSW_AF_EPF_FLR_DONE_INT, reg);
...
static int psw_register_flr_done_interrupts(struct rvu_block *block, int off,
					    u16 max_evfs)
{
	struct rvu *rvu = block->rvu;
	int blkaddr = block->addr;
	...
	epfs = rvu->hw->psw->num_epfs;
	for (id = 0; id < max_evfs; id++) {
		sprintf(&rvu->irq_name[(off + id) * NAME_SIZE],
			"PSWAF EVFFLR_DONE%d", id);
		ret = psw_do_register_interrupt(block, off + id,
						psw_evf_flr_done_intr_handler,
						&rvu->irq_name[(off + id) *
						NAME_SIZE]);
		if (ret)
			return ret;
		rvu_write64(rvu, blkaddr, PSW_AF_EVFX_FLR_DONE_INT_ENA_W1S(id),
			    INTR_MASK(epfs));
	}
	ret = psw_do_register_interrupt(block, off + id,
					psw_epf_flr_done_intr_handler,
					"PSWAF EPFFLR_DONE");
	if (ret)
		return ret;
	rvu_write64(rvu, blkaddr, PSW_AF_EPF_FLR_DONE_INT_ENA_W1S,
		    INTR_MASK(epfs));
				  ...
				  struct msg_rsp *rsp)
{
	...
	pf = rvu_get_pf(req->hdr.pcifunc);
	epf = psw->pf2epf_map[pf];
	port = (epf >= PSW_EPFS_PER_PORT) ? 1 : 0;
	epf = (epf >= PSW_EPFS_PER_PORT) ? (epf - PSW_EPFS_PER_PORT) : epf;
...
int rvu_psw_epf_teardown(struct rvu *rvu, u16 pcifunc, int blkaddr)
{
	...
	epf = psw->pf2epf_map[pf];
	port = (epf >= PSW_EPFS_PER_PORT) ? 1 : 0;
	port_epf = (epf >= PSW_EPFS_PER_PORT) ? (epf - PSW_EPFS_PER_PORT) : epf;
...
static void rvu_psw_unregister_interrupts_block(struct rvu_block *block, void *data)
{
	...
	for (i = 0; i < max_evfs; i++)
		rvu_write64(rvu, blkaddr, PSW_AF_EVFX_FLR_DONE_INT_ENA_W1C(i),
			    INTR_MASK(hw->psw->num_epfs));
	rvu_write64(rvu, blkaddr, PSW_AF_EPF_FLR_DONE_INT_ENA_W1C,
		    INTR_MASK(hw->psw->num_epfs));
...
static int rvu_psw_register_interrupts_block(struct rvu_block *block, void *data)
{
	...
	ret = psw_register_flr_done_interrupts(block, off, max_evfs);
	if (ret)
		goto err;
